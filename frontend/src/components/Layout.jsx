...
function Layout({ children }) {
  ...
  const { userId, userName, userRole, logout, authLoading } = useUser()
        ...
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-3 px-4 sm:px-6 lg:px-8">
          <div className="flex min-w-0 flex-1 items-center gap-4 md:gap-8">
            <Link to="/" className="flex min-w-0 shrink-0 items-center gap-2 sm:gap-3" onClick={() => setMobileOpen(false)}>
              <motion.span
                layout
                className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-violet-600/20 to-gray-900/40 p-1.5 shadow-lg shadow-violet-500/20 ring-1 ring-violet-400/25"
