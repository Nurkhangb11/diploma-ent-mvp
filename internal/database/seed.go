...
func SeedAdminUser() {
	...
	if err := DB.Where("email = ?", "admin@example.com").First(&existingUser).Error; err == nil {
		if existingUser.Role != models.RoleAdmin {
			DB.Model(&existingUser).Update("role", models.RoleAdmin)
			log.Println("Updated existing admin@example.com to admin role")
		} else {
	...
	log.Println("Successfully seeded admin user: admin@example.com")
}
